...
export async function POST(req: Request) {
  try {
    ...
    const scrimId = String(body.scrimId ?? '')
    ...
    if (!scrim || !['OPEN'].includes(scrim.status)) return NextResponse.json({ error: 'Registration is closed.' }, { status: 400 })
    ...
    const registration = await db.registration.create({ data: { code: `KRV-${new Date().getFullYear()}-${Math.floor(100000 + Math.random() * 900000)}`, userId: user.id, scrimId, slotNumber: count + 1, amount: scrim.entryFee, playerData: JSON.stringify({ ign: body.ign || user.ign, gameUid: body.gameUid || user.gameUid, teamName: body.teamName || null }) } })
