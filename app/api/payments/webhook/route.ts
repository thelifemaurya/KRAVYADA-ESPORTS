  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (registrationId && session.payment_status === 'paid') {
        await db.registration.updateMany({ where: { id: registrationId, paymentStatus: { not: 'REFUNDED' } }, data: { paymentStatus: 'PAID', paymentRef: typeof session.payment_intent === 'string' ? session.payment_intent : session.id } })
      }
    }
