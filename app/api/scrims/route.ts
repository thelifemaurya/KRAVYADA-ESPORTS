...
const publicStatuses=['OPEN','FULL','LIVE','COMPLETED'] as const

export async function GET(req:Request) {
  const status=new URL(req.url).searchParams.get('status')
  const requested=status&&publicStatuses.includes(status as typeof publicStatuses[number]) ? status as typeof publicStatuses[number] : null
