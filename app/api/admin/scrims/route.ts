...
export async function POST(req: Request) {
  try {
    ...
    if (!title || !game || !category || [date, startTime, registrationDeadline].some(Number.isNaN) || !Number.isInteger(entryFee) || entryFee < 0 || !Number.isInteger(prizePool) || prizePool < 0 || !Number.isInteger(totalSlots) || totalSlots < 1) {
      return NextResponse.json({ error: 'Invalid scrim data.' }, { status: 400 })
    }
